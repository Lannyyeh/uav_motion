...
template <int _N>
TrajectoryGenerator<_N>::TrajectoryGenerator(ros::NodeHandle &nh, int dimension) : nh_(nh),
...
void TrajectoryGenerator<_N>::waypointsCallback(const uav_motion::waypointsGoalConstPtr &goal)
{
	...
	// TODO: rewrite time allocation function
	...
	// print segment time
	std::cout << "Segment time: ";
	for (double i : segment_times)
		std::cout << i << ' ';
